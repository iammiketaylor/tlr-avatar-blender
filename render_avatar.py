...
def render(
    ...
    Returns a dict with keys svg and png
    """
...
def render_avatar_svg(req: Dict[str, Any]) -> str:
    ...
    Hu equals total height in px divided by nine.
    Landmarks:
    ...
    # Convert girths to half widths in px
    ...
    # Guard so the drawing stays on screen
    maxW = w * 0.42
    ...
    # Torso left half outline with mild S curve
    torso_path_left = _path([
        ("M", cx, y_chin),
        ("C",
            cx - shoulderW, y_chest - 0.50 * Hu,
            cx - chestW * 1.02, y_chest,
            cx - chestW * 1.00, y_chest + 0.25 * Hu
        ),
        ("S",
            cx - waistW * 0.95, y_waist,
    ...
    armL = _capsule_tapered(
        x1=cx - shoulderW, y1=y_shoulder,
        x2=cx - shoulderW - upperArmW * 0.35, y2=y_elbow,
        r1=upperArmW * 0.5, r2=foreArmW * 0.45,
        end_y=y_wrist
    )
    ...
    # Mirror torso to close body
    torso_path_right = mirror_path_h(torso_path_left, cx)
    torso_path = f"{torso_path_left} {torso_path_right} Z"
...
def _capsule_tapered(
    ...
    If end_y is provided it becomes the far end Y
    """
    ...
    return _path([
        ("M", x1 - r1 * 0.5, y1),
        ("C", cx - r1, cy, cx - r2, cy, x2 - r2 * 0.5, y_far),
        ("L", x2 + r2 * 0.5, y_far),
...
def mirror_path_h(path_d: str, x_axis: float) -> str:
    """
    Mirror a path string horizontally around x equals x_axis
    Assumes absolute commands and numeric coordinates in pairs
    """
    import re
...
def _grid_lines(w: int, h: int, Hu: float) -> str:
    """Draw faint horizontal guides and a vertical center line."""
